...
def copy_again(root_dir):
    """Copy last selection again."""
    if root_dir == '.':
        root_dir = os.getcwd()

    selection_file = os.path.join(root_dir, 'prompting', 'cli', 'last_selection.json')
    ...
    structure = generate_directory_structure(root_dir, ignore_patterns, always_ignore)
    structure_content = [
    ...
    ctx_path = os.path.join(root_dir, 'prompting', 'cli', 'ctx.xml')
